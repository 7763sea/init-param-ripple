...
namespace Ripple.Core.Crypto.Sjcljson
{
    public class JsonEncrypt
    {
        ...
        public JObject Encrypt(string key, string blob, string adata,byte[] salt,byte[] iv)
        {
            var result = new JObject();

            try
            {
                byte[] plainBytes = Encoding.UTF8.GetBytes(blob);
                byte[] adataBytes = Encoding.UTF8.GetBytes(adata);
                byte[] nonce = ComputeNonce(iv, plainBytes);
        ...
        public KeyParameter CreateKey(string key, JObject json)
        {
            try
            {
